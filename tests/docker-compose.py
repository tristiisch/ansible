...
def load_inventory(ansible_inventory_path: str) -> Dict[str, Any]:
	"""
	Load inventory from either a YAML file or a directory containing multiple YAML files.
	"""
	if os.path.isdir(ansible_inventory_path):
		return load_and_merge_yaml_from_dir(ansible_inventory_path)
	elif os.path.isfile(ansible_inventory_path):
		return load_yaml(ansible_inventory_path)
	else:
		raise FileNotFoundError(f"Path not found: {ansible_inventory_path}")

def load_and_merge_yaml_from_dir(directory: str) -> Dict[str, Any]:
	"""
	Load and merge all YAML files from a directory.
	Files are merged in alphabetical order (last one overrides previous keys).
	"""
...
def load_yaml(file_path: str) -> Dict[str, Any]:
	...
	with open(file_path, 'r', encoding='utf-8') as file:
		...
		if not isinstance(data, dict):
			raise ValueError(f"The file {file_path} does not contain a valid YAML dictionary")
	return data
...
def extract_hosts(inventory: Dict[str, Any]) -> list[str]:
	"""Extract the hosts names from the inventory."""
	hosts_keys: list[str] = []
	...
	for _, value in inventory.items():
		...
		for sub_key, sub_value in value.items():
			if isinstance(sub_key, str) and sub_key.casefold() == "hosts" and isinstance(sub_value, dict):
				hosts_keys.extend(sub_value.keys())
...
def __resolve_multiple_host(host_key: str) -> list[str]:
	...
	expanded_strings = [host_key]
	for match in matches:
		start, end = int(match[0]), int(match[1])
		new_expanded_strings = []
		for i in range(start, end + 1):
			for existing_string in expanded_strings:
...
def add_services_to_compose(docker_compose: Dict[str, Any], docker_services_settings: list[Dict[str, Any]] | None, docker_hosts: list[str]) -> None:
	"""Add services for each docker host to the Docker Compose structure."""
...
def main() -> None:
	parser = argparse.ArgumentParser(description="Convert Ansible inventory to Docker Compose")
	parser.add_argument("-i", "--input", required=True, help="Path to Ansible inventory YAML file or directory")
	parser.add_argument("-s", "--service-base", default=None, help="Comma-separated paths of service base settings")
	parser.add_argument("-o", "--output", default=None, help="Output Docker Compose YAML file")
	...
	inventory_data = load_inventory(ansible_inventory_path)
